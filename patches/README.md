...
- `0008-altsurgery-null-filename-guard.patch` — **not Mac-specific; upstream to `SimPE-Fixed` then retire this.** Adds `if (package.FileName == null) return false;` (and a `prov == null` guard) to `SurgeryTool2.IsEnabled` in Theo's Alt Sim Surgery, so `Helper.IsNeighborhoodFile(null)` can't NRE when a brand-new unsaved package is the active selection. Exactly the `// <- REQUIRED FIX` guard the built-in `SimPE.Toolbox/SurgeryTool.cs` already has; the decompiled Alt port was missing it. Touches only `SimPE.AltSurgery/SurgeryTool.cs`.
